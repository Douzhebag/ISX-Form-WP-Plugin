#!/usr/bin/env bash
# Install the WordPress PHPUnit test suite (core + tests-lib + test DB).
#
# Usage:
...
#
# Based on the standard `wp scaffold plugin-tests` script.
	...
	if [[ $WP_VERSION == 'nightly' || $WP_VERSION == 'trunk' ]]; then
		...
		if [ $WP_VERSION == 'latest' ]; then
			...
			if [[ $WP_VERSION =~ [0-9]+\.[0-9]+\.[0] ]]; then
				...
				# otherwise, scan the releases and get the most up to date minor version
...
install_db() {
	...
	then
		...
		create_db
		echo "Installed the test database ($DB_NAME)."
	fi
}
